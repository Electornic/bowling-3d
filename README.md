...
브라우저에서 돌아가는 3D 볼링 게임 (데스크톱·모바일 터치 지원). 정식 10프레임 룰(스트라이크/스페어 보너스), 슬립 기반 훅(스핀) 물리, 난이도별 오일 프리셋(하우스/숏/롱 + 프레임별 레인 마름), 네온/신스웨이브 볼링장(애니메이션 전광판·옆벽 광고판)까지 구현했다. **풀게임/블리츠/스페어 챌린지** 모드, **AI 라이벌 3인**(난이도 사다리) 대결과 **로컬 2인 교대전(hotseat)**, 스트라이크 **리플레이·스틸컷** 연출을 지원한다. 승리로 **업적**을 달성하면 **코스메틱 볼 스킨**이 해금된다(컬렉션 시트 — 외형 전용·물리/AI 사다리 무영향, 과금·가챠 없음). **외부 에셋 0개** — 도형·텍스처·사운드·UI 전부 코드로 생성.
...
- **스키드 → 훅 → 롤 3단계 궤적**: 볼링공이 휘는 건 마그누스가 아니라 지면 동마찰. 슬립 기반 측면력을 매 스텝 `applyImpulse(F·dt)`로 주입하고, 오일 존(하우스 기준 앞 10.5m — 난이도 프리셋·프레임별 마름으로 가변)/드라이 존 마찰 차등으로 실제 볼링처럼 막판에 꺾인다.
...
- **절차적 네온 볼링장**: 옆 레인·벽·천장 조명·옆벽 광고판·핀 뒤 애니메이션 전광판(신스웨이브 선셋+스크롤 그리드)까지 캔버스로 그려 에셋 없이 분위기. 스트라이크/스페어/거터는 전광판에 큼지막하게 어나운스하고, 스트라이크는 리플레이 후 점수판 아래 풀와이드 스틸컷 밴드로 마무리한다.
- **UI 네온 통일**: 점수판·볼무게·파워·스핀 오버레이를 공통 토큰(`src/ui/theme.ts`)으로 씬과 같은 네온 글래스 룩으로 통일.

## 문서

**설계 · 결정 (현행)**
- [docs/GAME_DESIGN.md](docs/GAME_DESIGN.md) — 설계 도안: *어떻게 만들어졌나* (좌표계·물리 상수·상태머신·Rapier API 검증 부록)
- [docs/DECISIONS.md](docs/DECISIONS.md) — 설계 결정 기록: *왜 이렇게 했나* (밸런스·스핀·마찰·AI·UI 근거 + 검증된 불변식)
...
**아카이브 (세션 로그)**
- [docs/archive/](docs/archive/) — 세션별 진행 기록·튜닝 로그·완료된 폴리싱 백로그. 결정 근거는 DECISIONS.md로, 구조는 GAME_DESIGN.md로 승격됨.
