...
export class SoundManager {
  ...
  set enabled(v: boolean) {
    ...
    if (!v) {
      ...
      if (this.ctx && this.psGain) {
        const now = this.ctx.currentTime;
        this.psGain.gain.cancelScheduledValues(now);
        this.psGain.gain.setTargetAtTime(0, now, 0.02); // 핀세터 모터도 즉시 멎게
      }
  ...
  // --- 핀세터 (도안 §6 연출) ---
  // 실제 볼링장에서 핀세터의 정체성은 절반이 소리다. 기계는 18m 밖이라 화면에서 몇 px밖에 안 되지만
  // 소리는 거리와 무관하게 100% 전달된다 — 같은 노력 대비 '기계가 돌고 있다'를 가장 크게 판다.
  // 전부 절차적 합성(에셋 0). PinSet의 구간 전환 콜백이 Boot를 거쳐 여기로 온다.

  private psGain: GainNode | null = null;

  /**
   * 모터 지속음. 사이클 내내 낮게 웅웅거리다 끝나면 사라진다.
   * 오실레이터 2개를 비배음 관계(62/93Hz)로 겹쳐 맥놀이를 만든다 — 순수 톤은 '삐-'로 들리고
   * 모터로 안 들린다. 시작/정지는 게인만 움직여 클릭을 막는다(setRoll과 같은 방식).
   */
  setPinsetter(on: boolean) {
    if (!this.ctx) return;
    const ctx = this.ctx;
    if (!this.psGain) {
      const g = ctx.createGain();
      ...
      const lp = ctx.createBiquadFilter();
      lp.type = 'lowpass';
      lp.frequency.value = 430; // 저역만 — 멀리 있는 기계는 고역이 먼저 죽는다
    ...
    const now = ctx.currentTime;
    const target = on && this.enabled ? 0.05 : 0;
    this.psGain.gain.cancelScheduledValues(now);
    this.psGain.gain.setTargetAtTime(target, now, on ? 0.09 : 0.2); // 끌 때 더 길게(여운)
  }

  /** 테이블이 핀을 물러 바닥에 닿는 순간의 금속 쿵 (②→③ 반환점) */
  playTableClunk() {
    if (!this.ctx || !this.enabled) return;
    const ctx = this.ctx;
    const now = ctx.currentTime;
    ...
    o.frequency.exponentialRampToValueAtTime(48, now + 0.13); // 떨어지는 피치 = 무거운 것이 닿음
    const g = ctx.createGain();
    ...
    o.start(now);
    o.stop(now + 0.18);
    this.metalTick(now, 0.055); // 금속 성분 — 저역 쿵만이면 '북'으로 들린다
  }

  /** 스윕 바가 데크를 훑는 긁는 소리 (④, 약 1초) */
  playSweepRake() {
    if (!this.ctx || !this.enabled) return;
    const ctx = this.ctx;
    const now = ctx.currentTime;
    const dur = 0.95; // CY_SWEEP 구간 길이와 맞춤
    ...
    const src = ctx.createBufferSource();
    src.buffer = buf;
    // 밴드패스를 위로 쓸어올렸다 내린다 = 바가 멀어졌다 가까워지는 도플러/질감 변화
    const bp = ctx.createBiquadFilter();
    bp.type = 'bandpass';
    bp.Q.value = 1.1;
    bp.frequency.setValueAtTime(620, now);
    bp.frequency.exponentialRampToValueAtTime(2100, now + dur * 0.7);
    bp.frequency.exponentialRampToValueAtTime(880, now + dur);
    const g = ctx.createGain();
    g.gain.setValueAtTime(0.0001, now);
    g.gain.exponentialRampToValueAtTime(0.045, now + 0.14);
    g.gain.setValueAtTime(0.045, now + dur - 0.22);
  ...
  /** 핀이 데크에 놓이는 딸깍 — 10개가 동시에 닿지 않으므로 짧게 흩뿌린다 (⑥ 끝) */
  playPinsSet(count = 10) {
    if (!this.ctx || !this.enabled) return;
    const ctx = this.ctx;
    const now = ctx.currentTime;
    const n = Math.max(1, Math.min(10, count));
    for (let i = 0; i < n; i++) {
      const t = now + Math.random() * 0.09; // 9ms 창에 흩뿌림 = 한 덩어리지만 기계적이지 않게
      const o = ctx.createOscillator();
      o.type = 'triangle';
      o.frequency.setValueAtTime(210 + Math.random() * 70, t);
  ...
  /** 짧은 금속 틱 — 쿵의 고역 성분 (단독으로는 안 쓴다) */
  private metalTick(at: number, vol: number) {
    const ctx = this.ctx!;
