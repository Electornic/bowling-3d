 ...
 *
 * 단계는 AMF 82-30 순서(PinSet.ts 상단 타임라인)를 그대로 따른다. 큐는 **그 단계가 시작되는 순간** 한 번 오고,
 * `dur`가 그 단계의 길이다 — 사운드는 단계 동안 울릴 지속음(모터·활주)을 그 길이로 예약하고, 직전 단계가 끝나며
 * 나는 타격음(바가 끝에 닿는 '쿵', 핀이 놓이는 '탁')을 시작 시각에 얹는다.
 *
 *   guard  ① 스윕 바가 가드 위치로 하강           grip   ② 테이블 하강, 선 핀의 목을 문다(리스팟만)
 *   lift   ③ 테이블이 핀을 들고 상승(리스팟만)     sweep  ④ 스윕 전진 — 데드우드를 피트로
 *   return ⑤ 스윕 가드 복귀                        set    ⑥ 테이블 하강 — 핀을 스폿에 놓음
 *   raise  ⑦ 테이블·스윕 상승                      done   사이클 종료. cut=true면 연출이 잘린 것(투구가 먼저 시작) —
 *                                                         모터만 즉시 끊고 '기계 귀환' 쿵은 내지 않는다
 */
...
export interface MachineCue {
  ...
  /** 이 단계의 길이(초). done은 0. */
  dur: number;
  /** set·raise에서 놓이는 핀 수(리스팟은 들고 있던 수, 새 랙은 10). 그 외 0. */
  pins: number;
