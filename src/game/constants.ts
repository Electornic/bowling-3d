 ...
 */
// 병 실루엣 (반지름 m, 바닥에서의 높이 m). USBC 규격에 맞춤 — 구 프로파일은 최대 지름이 76mm로
// 규격(121mm)의 63%밖에 안 돼 홀쭉한 스킷틀처럼 보였다(콜라이더 PIN_RADIUS 0.06=120mm와도 불일치).
//   높이 15in=380mm · 최대 지름 4.75in=121mm(바닥에서 ~114mm) · 목 최소 1.8in=46mm(바닥에서 ~254mm)
//   · 바닥 지름 2.25in=57mm.
// 0.268/0.288/0.300/0.320 지점은 목 빨간 띠의 경계 — 정점색이 또렷하게 맺히도록 일부러 점을 박아뒀다
// (PIN_STRIPES와 쌍으로 유지할 것).
export const PIN_PROFILE: readonly (readonly [number, number])[] = [
  [0.0, 0.0], [0.0285, 0.0], [0.0300, 0.010], [0.0450, 0.040], [0.0570, 0.080],
  [0.0605, 0.114], [0.0570, 0.150], [0.0480, 0.190], [0.0370, 0.225], [0.0265, 0.248],
  [0.0229, 0.254], [0.0235, 0.262],
  [0.0243, 0.268], [0.0255, 0.288],
  // 0.291·0.297 = 두 띠 사이의 '흰 간격'을 만드는 점. 이게 없으면 0.288(빨강)과 0.300(빨강)이
  // 보간되며 사이까지 빨개져 두 줄이 한 덩어리로 뭉친다(실측으로 확인).
  [0.0257, 0.291], [0.0261, 0.297],
  [0.0263, 0.300], [0.0280, 0.320],
  [0.0295, 0.336], [0.0280, 0.356], [0.0200, 0.372], [0.0090, 0.379], [0.0, 0.380],
];
/** 목 빨간 띠 구간 [y0, y1] (m, 바닥 기준). PIN_PROFILE에 경계점이 박혀 있어야 또렷하다. */
export const PIN_STRIPES: readonly (readonly [number, number])[] = [
  [0.268, 0.288],
  [0.300, 0.320],
];
