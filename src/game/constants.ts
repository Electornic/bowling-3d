...
// --- 전광판 무드 (B: 핀 뒤 전광판 = 비텍스트 반응형 무드 스크린, Environment.drawScreen) ---
// 스코어 이벤트를 텍스트가 아니라 "에너지"로만 반영 → 스틸컷(텍스트)과 역할 분리, 서로 안 겹침.
// energy=지속 열기(스트라이크 연속으로 상승 후 자연 냉각) · flash=순간 버스트 · dip=거터 브라운아웃.
export const MOOD = {
  baseEnergy: 0.15, // 아이들 열기 바닥 (0=완전 정적, 1=풀 온파이어)
  streakStep: 0.22, // 스트라이크 연속 1당 열기 (double 0.37 · turkey 0.59 · 4연속 0.81 · 5+ 1.0)
  energyTau: 2.5, // 열기 냉각 시상수(s) — 스트라이크 멈추면 이만큼에 걸쳐 base로 식음
  spareFlash: 0.5, // 스페어 순간 버스트 세기 (스트라이크=1.0 고정)
  flashTau: 0.7, // 플래시(스트라이크/스페어) 감쇠 시상수(s)
  dipTau: 0.45, // 거터 브라운아웃 감쇠 시상수(s)
  tintTau: 0.7, // 플레이어 색 크로스페이드 시상수(s)
  idleBreath: 0.03, // 아이들 호흡(느린 밝기 맥동) 진폭
} as const;
