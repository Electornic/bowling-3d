...
검증: tsc 클린 + vitest 22/22 + 프리뷰 격리 재측정(빈칸·채운칸 / 320·393px / 솔로·멀티 전부 한 줄·넘침 0). **남은 것: iPhone 실기 재확인 → 10차분과 묶어 커밋.**

## 10차 세션에 한 일 (인게임 UI 개선 — 미커밋)

> 상세 설계·진단·레퍼런스·결정·체크리스트는 [UI_REVAMP.md](./UI_REVAMP.md). 전부 `src/input/Controls.ts` + 문서, **미커밋·실기기(coarse) 미검증**(프리뷰가 마우스 분기만 보여줌).
...
검증: tsc 클린 + 데스크톱 프리뷰. **남은 것: iPhone 실기 확인(조준선 안정/곡률·폰 도크 46vw) → 묶어서 커밋.**
...
1. **점수판(11차) iPhone 실기 확인** — 미커밋·미검증. 인게임 HUD는 프리뷰로 직접 못 봐서 CSS 격리 측정만 했음. 폰에서 ① 솔로 10프레임 한 줄 가독성 ② 멀티 활성 풀 시트 + 비활성 미니(`Hud.renderMini`) 레이아웃 확인.
2. **묶어서 커밋** — 10차(P1~P4+조준선, iPhone 확인 완료) + 11차(점수판) = `Controls.ts`+`Hud.ts`+문서. 점수판까지 OK면 한 커밋으로.

**그 다음 (선택)**: ③ 승리 보상(설계 초안 완료 → [REWARDS.md](./REWARDS.md): 업적 뱃지 + 코스메틱 볼 스킨, 스탯 불가침), ④ 절차적 에셋 다듬기, P3 숙련 깊이. (거터 벽 점멸은 9차에 z-fighting으로 해결 완료. ⑥ 충돌 임팩트는 핀 fly-out·사운드로 완료.) 손맛: 팔로우 카메라(`CameraRig.ts`)·조준 감도(`AIM_RANGE` 0.08).
