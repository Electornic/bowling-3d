...
> 마지막 갱신: 2026-06-17 (15차 세션 — 구현 상황 점검 + 문서 정합: 13·14차가 `f46e7be`로 커밋 완료됐는데 문서에 "미커밋"으로 남아 있던 표기 일괄 정정. tsc 0·vitest 32/32·작업트리 클린·`develop`=`origin/develop` 동기·`main`보다 1커밋(`f46e7be`) 앞섬 확인)
...
- **남은 것**: AI 이름 확정(여전히 임시 흥부/홍길동/놀부) · iPhone 실기 검증 · (큰 거) P3 릴리스 타이밍 · 보상 P5/bloom · ④ 실물 에셋. (커밋은 `f46e7be`로 완료.)
...
2. **AI 이름 확정** — 13차에 여러 톤 돌렸으나 미확정(현재 코드=흥부/홍길동/놀부). 네온 톤 후보 **정석·제로·올인** / **PULSE·BLADE·JOKER**. (※ [DEV] 해금줄 제거·전부 커밋은 `f46e7be`로 완료.)
