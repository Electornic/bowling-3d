...
## 11. ⚠️ 미해결 설계 결정 — 구현 전 확정 (2차 검토)

2차 독립 검토(서브에이전트가 실제 코드 대조)가 찾은, 슬라이스 1 전에 결정해야 할 공백. 우선순위순.

| 항목 | 문제 | 결정 방향 | 근거 |
|---|---|---|---|
| H1 릴리스 페이즈 키잉 | 마지막프레임 보너스·AI 보너스 투구 미반영 | 릴리스 페이즈는 **매 `AIMING→ROLLING`** 발동(프레임당 1회 아님) | [GameState.ts:436](../src/game/GameState.ts), :308 |
| H2 AI 볼러 | AI는 입력경로 미경유·위치 없음 | AI 볼러는 위치만 배치, **스윙은 코스메틱(물리는 `computeAiThrow`)** | [ai.ts](../src/game/ai.ts) |
| H3 핫시트 ↔ 로비 | 핸드오프 오버레이·통계 미저장 정책 충돌 | **2인전은 로비 미경유·게임장 잔류** + 통계 미저장 유지 | [GameState.ts:551](../src/game/GameState.ts) |
| H4 로비 걸을 영역 | Environment 무콜라이더, 어프로치 바닥 z≈-5.5 깊이 7뿐 | **로비 바닥 범위 + 클램프 사각형 신규 정의** | [Environment.ts:167](../src/scene/Environment.ts) |
| M1 릴리스 삽입점 | `throwBall`이 즉시 `ROLLING` flip | **`RELEASING` 서브상태/게이트를 `launch` 전에** (카메라 case가 아니라 코드 변경) | [GameState.ts:231](../src/game/GameState.ts) |
| M2 스윙 timeScale | 슬로모/패스트포워드와 desync | 스윙을 **sim-time으로** 구동 또는 timeScale 제외 | [GameState.ts:290](../src/game/GameState.ts), [Loop.ts](../src/core/Loop.ts) |
| M3 `toLobby()` | 오일/레인 시각 리셋 미정의(현 `toMenu`는 안 함) | toLobby에서 오일/레인 시각 상태 결정 | [GameState.ts:192](../src/game/GameState.ts) |
| M4 CameraRig | 아바타 참조 없음·MENU 스웨이와 공존 | CameraRig에 **아바타 주입** + MENU↔LOBBY 정리 | [CameraRig.ts](../src/camera/CameraRig.ts) |
