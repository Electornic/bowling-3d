...
> 아래 "이벤트별 음원" 표의 이벤트는 **2026-09-03 기준 전부 구현됐거나 미채택으로 확정됐다** — 릴리스·굴림 ✓,
> 공-핀 충돌 ✓(투구당 1회, 서 있던 핀 수), 핀-핀 딸각 ✗(미채택), 스트라이크 '환호' ✗(스틸컷은 무음 — PA 차임·도장 쿵
> 둘 다 기각, 리플레이의 크래시 재발화가 대신한다), 거터 ✓('덜컥' + 홀로우 굴림음), UI 클릭 ✓(콘솔 스위치 어휘).
> 현재 소리 13종과 근거는 [SOUND.md §2](SOUND.md), 백로그 §5는 비었다.
